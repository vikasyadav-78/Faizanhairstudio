...
export default function CTA() {
  const handleBookClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    e.preventDefault();
    const contactSection = document.getElementById("reservations");
    if (contactSection) {
      ...
      <div className="max-w-5xl mx-auto px-6 text-center relative z-10">
        ...
        >
          <motion.a
            ...
            className="w-full sm:w-auto px-8 py-4 bg-gold-metallic hover:shadow-gold-glow-hover text-primary font-poppins font-bold text-xs uppercase tracking-widest border border-secondary transition-all duration-300 relative overflow-hidden group text-center rounded-lg"
          >
          ...
          <motion.a
            ...
            whileTap={{ scale: 0.98 }}
            href="tel:8171885432"
            className="w-full sm:w-auto px-8 py-4 border border-white/20 hover:border-secondary text-white hover:text-secondary font-poppins font-bold text-xs uppercase tracking-widest transition-all duration-300 flex items-center justify-center space-x-2 bg-black/40 backdrop-blur-sm text-center rounded-lg"
          >
            <Phone size={14} />
